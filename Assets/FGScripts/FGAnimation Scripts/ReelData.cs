...
namespace FGAnimations
{
    interface IReel<T>
    {
        public ReelCel<T> Read(int Time);
    }
    ...
    //TODO null cases
    [System.Serializable]
    public class ReelData<T> : IReel<T>
    {
        bool isExactTimeMode;
        ReelCel<T>[] _celData;

        public ReelCel<T> this[int time] { get { return Read(time); } }
        ...
        public ReelData()
        {
            isExactTimeMode = false;
        }
        public ReelData(T[] data, int TimeInbetween = 1, bool isExact = false)
        {
            Add(data, TimeInbetween);
            isExactTimeMode = isExact;
        }
        /// <summary>
        ...
        public ReelCel<T> Read(int Time)
        {
            Assert.IsNotNull(_celData);

            if (isExactTimeMode)
            {
                return ExactRead(Time);
            }
            else
            {
                return NearestRead(Time);
            }

        }
        ReelCel<T> ExactRead(int Time)
        {
            for (int x = 0; x < _celData.Length; x++)
                if (_celData[x].TimePoint == Time)
                    return _celData[x];

            return null;
        }
        ReelCel<T> NearestRead(int Time)
        {
            if (ReelLength == 1 || Time < _celData[0].TimePoint)
                return _celData[0];
        ...
        void SaveNewData(ReelCel<T>[] Data)
        {
            _celData = Data;
        }
    ...
    public class EmptyCel<T> : ReelCel<T>
    {
        public EmptyCel(T data, int Point) : base(data, Point)
        {
            IsNull = true;
        }
